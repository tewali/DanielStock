...
const CSS = `
.ck { color:${T.ink}; background:${T.paper}; font-family: ui-sans-serif, system-ui, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; font-size:14px; line-height:1.45; -webkit-font-smoothing:antialiased; }
...
.ck button { font:inherit; color:inherit; cursor:pointer; }
...
.ck th { text-align:left; font-weight:600; font-size:11.5px; color:${T.muted}; padding:7px 10px; border-bottom:1px solid ${T.line}; white-space:nowrap; background:${T.panel}; position:sticky; top:0; z-index:2; }
.ck th.s { cursor:pointer; user-select:none; }
.ck th.s:hover { color:${T.ink}; }
.ck td { padding:7px 10px; border-bottom:1px solid #EBEEEA; vertical-align:middle; }
.ck tbody tr:hover td { background:#F6F8F5; }
...
.ck input[type=number], .ck input[type=text], .ck select { border:1px solid ${T.line}; background:${T.panel}; border-radius:3px; padding:4px 6px; font-variant-numeric:tabular-nums; }
.ck input[type=range] { accent-color:${T.accent}; width:100%; }
.ck .navbtn { display:block; width:100%; text-align:left; background:none; border:0; padding:7px 10px 7px 12px; border-left:2px solid transparent; color:${T.muted}; border-radius:0; }
.ck .navbtn:hover { color:${T.ink}; background:#E6E9E3; }
.ck .navbtn[aria-current=true] { color:${T.ink}; border-left-color:${T.accent}; background:#E4E8E1; font-weight:600; }
.ck .pill { display:inline-block; padding:1px 7px; border-radius:2px; font-size:11.5px; font-weight:600; white-space:nowrap; }
.ck .panel { background:${T.panel}; border:1px solid ${T.line}; }
...
function Cockpit({ m, open, selected, go }) {
  ...
  return (
    <>
      <div className="panel" style={{ display: "flex", flexWrap: "wrap", marginBottom: 22 }}>
...
function Kurse({ m, state, set }) {
  ...
  const run = async () => {
    setErr(""); setRes({}); setFxRes(null); setRunning(true); setDone(0);
    const ctrl = new AbortController();
    abort.current = ctrl;
    const batches = [];
    for (let i = 0; i < targets.length; i += 8) batches.push(targets.slice(i, i + 8));
    setTotal(batches.length + (withFx ? 1 : 0));

    const runBatch = async (batch) => {
      const lines = batch.map((t) => `${t.ticker} | ${t.name} | ${t.ccy} | Referenz ${t.base}`).join("\n");
      const prompt =
        "Suche den letzten verfügbaren Schlusskurs dieser Aktien im Web.\n\n" +
        "Ticker | Unternehmen | erwartete Währung | letzter bekannter Kurs\n" + lines +
        "\n\nRegeln:\n" +
        "- Kurs in der Währung der genannten Hauptnotiz, nicht umgerechnet.\n" +
        "- Kein belastbarer Kurs gefunden: Titel weglassen, nicht schätzen.\n" +
        "- Datum des Kurses und die Quelldomain angeben.\n\n" +
        'Antworte ausschließlich mit JSON, ohne Markdown und ohne Vorwort:\n' +
        '[{"ticker":"AAPL","price":123.45,"currency":"USD","asof":"2026-08-31","source":"finance.yahoo.com"}]';
      const txt = await askClaude(prompt, ctrl.signal);
      const arr = parseJson(txt);
      const add = {};
      arr.forEach((q) => {
        const t = targets.find((x) => x.ticker === (q.ticker || "").trim());
        if (!t || typeof q.price !== "number" || !(q.price > 0)) return;
        const dev = q.price / t.base - 1;
        const ccyOk = !q.currency || String(q.currency).toUpperCase() === t.ccy;
        add[t.ticker] = {
          ...t, price: q.price, dev,
          asof: q.asof || "–", source: q.source || "–",
          status: !ccyOk ? "waehrung" : Math.abs(dev) > DEV_LIMIT ? "abweichung" : "ok",
          returnedCcy: q.currency,
    ...
    try {
      ...
      const workers = Array.from({ length: Math.min(3, queue.length) }, async () => {
        while (queue.length) {
          const b = queue.shift();
          try { await runBatch(b); } catch (e) { setDone((d) => d + 1); setErr(e.message); }
        }
      });
      ...
      if (withFx) {
        try {
          const txt = await askClaude(
            "Wie viele US-Dollar kostet aktuell ein Euro (Wechselkurs EUR/USD)? Suche im Web.\n" +
            'Antworte ausschließlich mit JSON: [{"pair":"EURUSD","rate":1.0850,"asof":"2026-08-31","source":"domain"}]',
            ctrl.signal
          );
          const a = parseJson(txt)[0];
          if (a && typeof a.rate === "number" && a.rate > 0.5 && a.rate < 2) setFxRes(a);
        } catch (e) { /* FX bleibt unverändert */ }
        setDone((d) => d + 1);
      }
    } catch (e) {
      setErr(e.name === "AbortError" ? "abgebrochen" : e.message);
    }
    setRunning(false);
  };
  ...
  return (
    <>
      <Section
        ...
        note="Das Cockpit holt die Kurse über eine Websuche und schreibt sie als Kursstand in das Modell. Das ist ein Rechercheabgleich, kein Börsenfeed: Kurse können verzögert, gerundet oder von einer Zweitnotiz sein. Vor einer Order immer im Broker prüfen."
      >
        <div className="panel" style={{ padding: "12px 14px", display: "flex", flexWrap: "wrap", gap: 16, alignItems: "center" }}>
          ...
          <span className="lbl">{targets.length} Titel · {Math.ceil(targets.length / 8)} Suchläufe</span>
          <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12.5 }}>
            <input type="checkbox" checked={withFx} onChange={(e) => setWithFx(e.target.checked)} disabled={running} /> EUR/USD mitziehen
          </label>
          <button
            onClick={running ? () => abort.current && abort.current.abort() : run}
            style={{ border: 0, background: running ? T.sell : T.ink, color: T.paper, padding: "7px 14px", borderRadius: 3, fontWeight: 600 }}
          >
            {running ? "Abbrechen" : "Kurse holen"}
          </button>
          {live > 0 && (
        ...
        {(running || total > 0) && (
          <div style={{ marginTop: 12 }}>
            <div style={{ height: 6, background: "#E4E8E3" }}>
              <div style={{ height: "100%", width: total ? (done / total) * 100 + "%" : 0, background: T.accent, transition: "width 200ms ease" }} />
            </div>
            <div className="lbl" style={{ marginTop: 5 }}>
              {running ? `Suchlauf ${done} von ${total} · das dauert pro Lauf einige Sekunden` : `${done} von ${total} Läufen abgeschlossen · ${rows.length} Kurse gefunden`}
            </div>
          </div>
        )}
        {err && <div style={{ marginTop: 10, color: T.sell }}>Fehler: {err}. Einzelne Läufe können leer bleiben – nicht gefundene Titel behalten ihren alten Kurs.</div>}
          ...
          <>
            ...
            {fxRes && (
              <label className="panel" style={{ display: "flex", gap: 10, alignItems: "center", padding: "8px 12px", marginBottom: 10 }}>
                ...
                <span className="lbl">Stand {fxRes.asof} · {fxRes.source}</span>
              </label>
            )}
                ...
                <tbody>
                  {rows.map((r) => (
                    <tr key={r.ticker}>
                      ...
                      <td className="lbl">{r.asof}</td>
                      <td className="lbl" style={{ overflow: "hidden", textOverflow: "ellipsis" }}>{r.source}</td>
                      <td>
      ...
      <Section title="Wie der Abgleich funktioniert" note="Damit du weißt, worauf du dich verlässt.">
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(250px, 1fr))", gap: 14 }}>
          {[
            ["Woher die Kurse kommen", "Jeder Lauf schickt acht Titel an Claude, das im Web nach dem letzten Schlusskurs sucht und Kurs, Datum und Quelle zurückgibt. Ein direkter Börsenfeed ist aus dieser Oberfläche heraus nicht erreichbar."],
            ...
            ["Was der Abgleich verändert", "Übernommene Kurse ersetzen den Kursstand im ganzen Modell: Zonen, Abstände, Gewichte, Optionskandidaten und Kaufplan rechnen sofort neu. Zurücksetzen stellt die Arbeitsmappe wieder her."],
            ["Grenzen", "Verzögerte Kurse, Zweitnotierungen und ADR-gegen-Stammaktie sind die typischen Fehlerquellen. Für eine Order zählt der Kurs im Broker, nicht dieser Abgleich."],
          ].map(([h, t]) => (
  ...
  return (
    ...
    >
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", padding: "14px 16px", borderBottom: `1px solid ${T.line}` }}>
        ...
        <button onClick={close} aria-label="schließen" style={{ border: 0, background: "none", fontSize: 20, lineHeight: 1, color: T.muted }}>×</button>
      </div>
    ...
    <div className="ck" style={{ minHeight: "100vh" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start" }}>
        ...
        <main style={{ flex: 1, minWidth: 0, padding: "20px 22px 60px" }}>
          ...
          {tab === "kurse" && <Kurse m={m} state={state} set={set} />}
