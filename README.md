...
1. `.env.example` als `.env.local` kopieren, `DASHBOARD_PASSWORD` setzen und `DATABASE_URL` mit einer PostgreSQL-Verbindungs-URL befüllen. Für die bereitgestellte Cloudflare-Laufzeit sollte der Anbieter HTTP/WebSocket-Verbindungen unterstützen (z. B. Neon).
2. `npm install`
3. `npm run dev`
